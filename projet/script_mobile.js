...
function round(number, nbOfDecimals) {
  var factor = Math.pow(10, nbOfDecimals);
...
function cryptCaesarInput(text) {
  ...
  if (isNumeric(text)) {
    // a number
    offset = Math.abs(parseInt(text) % ALPHABET_SIZE);
  } else {
		if (DEFAULT_ALPHABET.indexOf(text) != -1) { // <- is letter
     	offset = Math.abs((text.charCodeAt(0) - DEFAULT_ALPHABET[0].charCodeAt(0)) % ALPHABET_SIZE);
    }
  }
...
function cryptSubstInput() {
  ...
  if (alpha.value == "") {
    ...
    if(randomAb.length !== s.size || randomAb.length != ALPHABET_SIZE){
      // @TODO
...
function drawBar(ctx, upperLeftCornerX, upperLeftCornerY, width, height, color, legend) {
  ...
  ctx.fillText(legend, -upperLeftCornerY + 25, upperLeftCornerX + 25);
  ctx.restore();
}
...
var Barchart = function(options) {
  ...
  this.draw = function(scale) {
    ...
    this.ctx.fillText(this.options.seriesName, -this.canvas.height / 2 - 30, 30);
    this.ctx.restore();
  }
}
...
function showFreqDiagram(id, data) {
  ...
  document.getElementById(id + "Canvas").style.webkitTransform = "rotate(90deg)";
}
...
function drawBigramTable(canvas, data, options) {
  ...
  ctx.font = "bold 30px Arial";
	...
	let offset = 40;
	let xsize = (canvas.width - offset) / ALPHABET_SIZE;
	let ysize = (canvas.height - offset) / ALPHABET_SIZE;
	...
	for (e of data) {
		...
		// check if the value isn't interessting -> set the color of the node to backgroundColor
		if (r >= 250 && g >= 250 && b >= 250) {
			color = options.backgroundColor;
		}

		// check if the color is very intressting
		if (r <= 10 && g <= 10 && b <= 10) {
			value = 10;
	...
	for (let i = 0; i < ALPHABET_SIZE; ++i) {
		...
		ctx.fillRect(offset / 2 + i * xsize, offset / 2, 1, offset / 2 + (ALPHABET_SIZE - 1) * ysize);
	}
	// draw the last bar
	ctx.fillRect(offset / 2 + ALPHABET_SIZE * xsize, offset / 2, 1, offset / 2 + (ALPHABET_SIZE - 1) * ysize);
	...
	for (let i = 0; i < ALPHABET_SIZE; ++i) {
		...
		ctx.fillStyle = options.lineColor;
		ctx.fillRect(offset / 2, offset / 2 + i * ysize, offset / 2 + (ALPHABET_SIZE - 1) * xsize, 1);
	}
	// draw the last bar
	ctx.fillRect(offset / 2, offset / 2 + ALPHABET_SIZE * ysize, offset / 2 + (ALPHABET_SIZE - 1) * xsize, 1);
}
...
function showDiv(){
  var selected = document.getElementById('textType');
  var choice = selected.selectedIndex;
...
function showselect() {
  ...
  let menu = document.getElementById('CipherMethod');
  var selected = document.getElementById('textType');
  var choice = selected.selectedIndex;
  var select = selected.options[choice].value;
  if (select == 0) {
    menu.style.display = 'block';
    choice = menu.selectedIndex;
    var selectcy = menu.options[choice].value;
    if (selectcy == 0) {
...
function TransformMenu() {
  ...
  let selected = document.getElementById('textType');
	let method = document.getElementById('CipherMethod');

	// reset intervall if we Go back
	if (menu.style.display == 'none') {
		selected.selectedIndex = 0;
		method.selectedIndex = 0;
	} else {
		// check if user select something
		if (selected.selectedIndex == 0 || (selected.selectedIndex == 1 && method.selectedIndex == 0)) {
			alert("Vous devez selectionner une methode de chiffrement et un type de texte pour poursuivre");
			return;
		}
	}
...
function randomTextDifficulty(tab) {
  let selected = document.getElementById('textType');
  let choice = selected.selectedIndex;
	let select = selected.options[choice].value;
  let randomint = Math.floor(Math.random() * tab.length);

  if (select == 1) {
    document.getElementById("TextInput").value = tab[randomint];
  } else if (select == 0) {
    selected = document.getElementById('CipherMethod');
    choice = selected.selectedIndex;
    CipherMethod = selected.options[choice].value;
...
function decryptAutomaticCesar() {
  ...
  if (encryptedText == "") {
    ...
    document.getElementById("keyAuto").value = maxI;
  }
}
...
function goAttaque() {
  ...
  let selected = document.getElementById('textType');
	selected.options[selected.selectedIndex].value = 0;
